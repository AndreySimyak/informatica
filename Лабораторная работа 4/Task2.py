...
def task() -> None:
    ...
    with open(OUTPUT_FILENAME, "w") as file:  # Открываем файл, чтобы запсать результаты в режиме редактированния.
        json.dump(line, file, indent=4)  # С помощью метода dump запишем полученные результаты в json-файл.
